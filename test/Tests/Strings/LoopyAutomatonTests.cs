...
namespace Microsoft.ML.Probabilistic.Tests
{
    ...
    public class LoopyAutomatonTests
    {
        ...
        {
            StringAutomaton automaton = StringAutomaton.Zero();
            automaton.Start.AddTransition('a', Weight.One).AddSelfTransition(default(DiscreteChar), Weight.FromValue(3)).SetEndWeight(Weight.One);
        ...
        {
            StringAutomaton automaton = StringAutomaton.Zero();
            automaton.Start.AddTransition('a', Weight.One).AddSelfTransition(default(DiscreteChar), Weight.FromValue(2)).SetEndWeight(Weight.One);
            automaton.Start.AddTransition('b', Weight.One).AddSelfTransition(default(DiscreteChar), Weight.FromValue(1)).SetEndWeight(Weight.One);
        ...
        private static double GetLogNormalizerByGetValue(StringAutomaton automaton)
        {
            ...
            epsilonAutomaton.SetToFunction(automaton, (dist, weight, group) => Tuple.Create<DiscreteChar, Weight>(default(DiscreteChar), weight)); // Convert all the edges to epsilon edges
